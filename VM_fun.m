function VM_fun(VM,ALT,XNC,ALT_fin,XNC_fin,WACT,TAUACT,ZETAACT,TAU,WCR,ZETA,WAF,ZAF,TF,k,handles)
%% Funcion buscar
...
m=0;
g=k;
VM_vector=[VM(1) VM(length(VM)/2) VM(length(VM))];

XNCG=XNC_fin;
for VM_fin=[VM(1) VM(length(VM)/2) VM(length(VM))]
if g==1
...
datastr1=strcat('VM = ',num2str(VM_vector(1)),'[ft/s]');

datastr2=strcat('VM = ',num2str(VM_vector(2)),'[ft/s]');

datastr3=strcat('VM = ',num2str(VM_vector(3)),'[ft/s]');
