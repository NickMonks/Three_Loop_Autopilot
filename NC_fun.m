function NC_fun(VM,ALT,...
    XNC,ALT_fin,VM_fin,WACT,TAUACT,ZETAACT,TAU,WCR,ZETA,WAF,ZAF,TF,k,handles)
%% Funcion buscar
...
m=0;
g=k;
NC_vector=[XNC(1) XNC(length(XNC)/2) XNC(length(XNC)) ];
for XNC_fin=[XNC(1) XNC(length(XNC)/2) XNC(length(XNC)) ]
if g==1
...
datastr1=strcat('XNC = ',num2str(NC_vector(1)),'[G]');

datastr2=strcat('XNC = ',num2str(NC_vector(2)),'[G]');

datastr3=strcat('XNC = ',num2str(NC_vector(3)),'[G]');
