function ZETAACT_fun(XNCG,XKA,XKDC,XKR,WI,TA,WZ,XK1,XK3,WACT,TAUACT,W0,Z0,XK0,WAF,ZAF,TF,handles,WCR)
m=0;
ZETAACT_vector=linspace(0.1,1.1,3);
for ZETAACT=ZETAACT_vector
count=0;
m=m+1;
E=0.;
ED=0.;
...
DELD_coef  = 2*ZETAACT/WACT + TAUACT;
DELDD_coef = 2*ZETAACT*TAUACT/WACT + 1/WACT^2;
DELDDD     = (WACT^2/TAUACT)*(DELC - DEL - DELD_coef*DELD - DELDD_coef*DELDD);
...
datastr1=strcat('ZETA act = ',num2str(ZETAACT_vector(1)),'[-]');

datastr2=strcat('ZETA act = ',num2str(ZETAACT_vector(2)),'[-]');

datastr3=strcat('ZETA act = ',num2str(ZETAACT_vector(3)),'[-]');
...
save resultadosZETA.mat W_0dB W_phase180 GAIN_180phase GAIN_WCR Gmar_1 Gmar_2 G_error...
